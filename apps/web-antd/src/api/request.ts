...
const isLogging = ref(false); // 防止并发登录请求
...
function createRequestClient(baseURL: string, options?: RequestClientOptions) {
  ...
  client.addRequestInterceptor({
    fulfilled: async (config) => {
      ...
      if (!accessStore.accessToken && !isLogging.value) {
        isLogging.value = true;
        try {
          const resp = await loginApi({ username: 'admin', password: 'admin123' });
...
export const baseRequestClient = new RequestClient({ baseURL: API_URL });
